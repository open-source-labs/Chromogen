...
export const Provider = () => {
  ...
  const copySelectors = (arr) =>
    arr.map(({ calls, key, newSelector }) => ({ calls: { ...calls }, key, newSelector }));
  ...
  useRecoilTransactionObserver_UNSTABLE(({ snapshot, previousSnapshot }) => {
    ...
    const currentState = atoms.map(
      (atom) => `${atom.key}: ${JSON.stringify(snapshot.getLoadable(atom).contents)}`,
    );

    // FIX: selectors fire AFTER transactionObserver, so count / filter is off-by-one
    const firedSelectors = selectors.filter(({ calls: { count }, key }) => {
    ...
    // LOGGING
    console.log('\n** PREVIOUS STATE **');
    previousState.forEach((atomStr) => {
      console.log(atomStr);
    });
    console.log('\n** POST-TRANSACTION STATE **');
    currentState.forEach((atomStr) => {
      console.log(atomStr);
    });
    console.log('\n** SELECTORS **');
    firedSelectors.forEach(({ calls: { count }, key }) => {
      console.log(`${key} fired (${count} total calls)`);
    });
    console.log('---------------');

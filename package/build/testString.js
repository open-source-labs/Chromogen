...
const output = (writeables, readables, snapshots, initialRender) => `import { renderRecoilHook, act } from 'react-recoil-hooks-testing-library';
...
const useStoreHook = () => {
  ${writeables.reduce((str, { key }) => `${str}const [${key}Value, set${key}] = useRecoilState(${key});\n`, '')}
...
describe('INITIAL RENDER', () => { 
  ...
  ${initialRender.reduce((initialTests, { key, newValue }) => `${initialTests}it('${key} should initialize correctly', () => {
      expect(result.current.${key}Value).toStrictEqual(${JSON.stringify(newValue)});
    });\n\n`, '')}
});

describe('SELECTORS', () => {
  ${snapshots.reduce((tests, { state, selectors }, index) => `${tests}it('State-${index + 1}', () => {
      const { result } = renderRecoilHook(useStoreHook);
      ...
      ${selectors.reduce((assertions, { key, newValue }) => `${assertions}expect(result.current.${key}Value).toStrictEqual(${JSON.stringify(newValue)});\n\n`, '')}
    });\n\n`, '')}
})`;
